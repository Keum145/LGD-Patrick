...
export async function POST(request: NextRequest) {
  try {
    ...
    const cacheKey = `ai-jobs:${normalize(company)}`;
    if (serverSupabase) {
    ...
    const result = await createStructuredResponse<AiJobResult>({
      ...
      instructions:
        "당신은 한국 채용 공고 일정 검증 도우미다. 무료 채용 검색에서 마감일을 확인하지 못했을 때만 호출된다. 공식 기업 채용 페이지, 자소설닷컴, 사람인, 잡코리아, 원티드, 캐치를 우선 확인한다. 현재 지원 가능한 공고만 반환하고 날짜와 URL을 추측하지 않는다. 마감일을 원문에서 확인할 수 없으면 null로 둔다. 날짜는 YYYY-MM-DD 형식만 사용한다. 상시채용은 deadline을 null로 둔다.",
      input: `오늘은 ${today}(한국 시간)이다. '${company}'의 현재 지원 가능한 신입·인턴·주니어 채용 공고를 찾아 정확한 접수 시작일과 서류 마감일을 확인하라. 종료된 공고와 해외 공고, 단순 기업 뉴스는 제외하라.`,
      schema,
    });
